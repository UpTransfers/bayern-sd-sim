...
function wagePressure(candidate: TransferCandidate) {
  ...
  return agePressure * marketPressure * starPressure;
}
...
export function evaluateBayernTransferApproval(candidate: TransferCandidate, summary: SimulationSummary): BayernTransferApproval {
  ...
  const wageMultiplier = wagePressure(candidate);
  ...
  if (budgetShare >= 0.45 && candidate.realism !== "Realistic") {
    vetoReasons.push("The fee would eat too much of the current budget for a non-core target.");
  }
  if (wageMultiplier >= 1.3 && candidate.realism !== "Realistic") {
  ...
  const sportingNeed = Math.round(
    clamp(
      25 * positionNeed * positionPriority +
  ...
  const supervisoryBoardAcceptance = Math.round(
    clamp(boardBase + (candidate.bayernFit >= 8.5 ? 2 : 0) + (bundesligaFamiliarity ? 1 : 0) - (pathwayBlock ? 2 : 0) - (roleBucket === "ATT" && feeHigh ? 1 : 0), 0, 15),
  );
  ...
  if (total >= 85) decision = "Approved";
  else if (total >= 70) decision = "Approved after negotiation";
  else if (total >= 55) decision = "Board review / delayed / depends on sales";
  else if (total >= 40) decision = "Likely rejected";
  else decision = "Unrealistic";

  const hardBlock =
    total < 40 ||
    (feeHigh && !guaranteedStarter && total < 68) ||
    (oldAndExpensive && total < 75) ||
    (pathwayBlock && total < 60) ||
    (wageMultiplier >= 1.35 && total < 72);

  if (hardBlock) {
    decision = total < 40 ? "Unrealistic" : "Likely rejected";
  } else if (vetoReasons.length) {
    if (decision === "Approved") decision = "Board review / delayed / depends on sales";
    if (total < 55) decision = "Likely rejected";
  }
